  ...
  <div class="content">      
    ...
    <div id="cadastro">
      <form @submit.prevent="savelivro()" method="post" action=""> 
        <h1>Livro {{this.$route.params.name}}</h1> 
        ...
        <p> 
          ...
          <input id="description" name="description" required="required" type="text"  placeholder="Uma das principais reflexões de 1984..." 
          maxlength="600" v-model="createlivro.description"/>
        </p>
...
import {Livro, Livroid} from '../services/resources.js';
...
export default {
  ...
  created() {
      ...
      Livroid.query({id: this.id_livro}).then(response => {
            this.createlivro.name = response.data.name,
            this.createlivro.author = response.data.author,
            this.createlivro.description = response.data.description
